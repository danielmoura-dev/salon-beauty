@extends('layouts.app')
@section('title', 'Clientes — Salon Beauty')
        ...
        @forelse ($clients as $client)
            <div class="flex items-center gap-4 rounded-2xl bg-white border border-gray-100 px-4 py-3 shadow-sm">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <form id="del-client-{{ $client->id }}"
                          method="POST" action="{{ route('clients.destroy', $client) }}">
                        @csrf @method('DELETE')
                        <button type="button"
                            @click="$dispatch('open-confirm', { formId: 'del-client-{{ $client->id }}', message: 'Remover {{ addslashes($client->name) }}? Esta ação não pode ser desfeita.' })"
