@extends('layouts.app')
@section('title', 'Produtos — Salon Beauty')
        ...
        @forelse ($products as $product)
            <div class="rounded-2xl bg-white border overflow-hidden flex flex-col shadow-sm
                ...
                <div class="relative aspect-square w-full bg-gray-50">
                    ...
                    @endif
                    @if ($product->for_sale)
                        <span class="absolute top-2 right-2 text-xs bg-green-500 text-white rounded-full px-2 py-0.5 font-semibold shadow-sm">
                ...
                <div class="p-3 flex flex-col flex-1">
                    <p class="font-semibold text-gray-900 text-sm truncate leading-tight">{{ $product->name }}</p>
                    ...
                    <div class="flex gap-1.5 mt-3 mt-auto">
                        <button @click="openEdit({{ $product->toJson() }})"
                            class="flex-1 inline-flex items-center justify-center gap-1 rounded-lg py-1.5 text-xs font-medium bg-gray-100 text-gray-600 hover:bg-primary-50 hover:text-primary-600 transition-colors">
                        ...
                        <form id="del-product-{{ $product->id }}"
                              method="POST" action="{{ route('products.destroy', $product) }}">
                            @csrf @method('DELETE')
                            <button type="button"
                                @click="$dispatch('open-confirm', { formId: 'del-product-{{ $product->id }}', message: 'Remover {{ addslashes($product->name) }}? Esta ação não pode ser desfeita.' })"
                                class="inline-flex items-center justify-center rounded-lg p-1.5 text-xs font-medium bg-gray-100 text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors">
