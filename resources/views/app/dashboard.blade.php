...
<div class="space-y-6">
    ...
    @if ($lowStockProducts->isNotEmpty())
        <div class="rounded-2xl bg-orange-50 border border-orange-200 p-4">
            <p class="text-sm font-semibold text-orange-700 mb-2 flex items-center gap-1.5">
                <svg class="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/></svg>
                Estoque baixo ({{ $lowStockProducts->count() }} {{ $lowStockProducts->count() === 1 ? 'produto' : 'produtos' }})
            </p>
            <div class="flex flex-col gap-1.5">
                @foreach ($lowStockProducts as $p)
                    <div class="flex items-center justify-between text-sm">
                        <span class="font-medium text-orange-900">{{ $p->name }}</span>
                        <span class="text-orange-700 font-semibold">{{ $p->stock_qty }} un.</span>
                    </div>
                @endforeach
            </div>
            <a href="{{ route('products') }}" class="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-orange-700 hover:underline">
                Ver produtos
