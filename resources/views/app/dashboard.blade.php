...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-4">
            <svg class="h-6 w-6 text-rose-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5"/></svg>
    ...
    @if ($tenant->plan_status === 'trial')
        <div class="rounded-2xl bg-rose-50 border border-rose-200 p-4 flex items-center gap-4">
            <svg class="h-6 w-6 text-rose-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
    ...
    @if ($nextAppointments->isNotEmpty())
        <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <a href="{{ route('agenda') }}" class="text-sm text-rose-600 hover:underline flex items-center gap-1">
                    Ver agenda
            ...
            @foreach ($shortcuts as $s)
                <a href="{{ route($s['route']) }}"
                   class="flex items-center gap-3 rounded-2xl bg-white border border-gray-100 px-4 py-3.5
                    ...
                    <svg class="h-5 w-5 text-gray-400 group-hover:text-rose-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">{!! $s['svg'] !!}</svg>
